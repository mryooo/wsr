...
function renderErosionServices() {
    ...
    const rows = inventoryIds.map(id => {
        ...
        const conditionDisplay = condition === 'weathered'
            ? (currentLang === 'ja' ? `風化・使用時${Math.round(nextConfig.misfire * 100)}%で不発・1個消費・状態継続` : `WEATHERED · ${Math.round(nextConfig.misfire * 100)}% MISFIRE · CONSUMES ONE · PERSISTS`)
            : condition === 'polluted'
    ...
    container.innerHTML = `<div class="glass-panel border border-purple-500/25 bg-purple-950/20 p-2 rounded-lg">
        ...
        <div class="text-[9px] text-white mt-1 mb-1">${!protectionAvailable && isBossFloor(gameState.floor + 1) ? (currentLang === 'ja' ? `次はボス階層のため侵食判定はありません。浄化は残り${EROSION_CLEANSE_LIMIT - cleansesUsed}回です。` : `The next boss floor has no erosion check. ${EROSION_CLEANSE_LIMIT - cleansesUsed} cleanses remain.`) : (currentLang === 'ja' ? `${nextConfig.allTargets ? '40層以降は未保護の全所持枠が侵食します。' : `侵食率 ${Math.round(nextConfig.rate * 100)}%。`}風化は使用時に最大${Math.round(nextConfig.misfire * 100)}%で不発になり1個消費、不発後も状態が継続します。汚染は次降下で1個消滅、腐敗は使用不能になり次降下で同種を全数失います。保護は次の1階層のみ、浄化は各ショップ2回まで・1回につき1段階です。` : `${nextConfig.allTargets ? 'From floor 40, every unsealed inventory slot erodes. ' : `Erosion ${Math.round(nextConfig.rate * 100)}%. `}Weathered items may misfire up to ${Math.round(nextConfig.misfire * 100)}%, consume one, and remain weathered; polluted items lose one on descent; decayed items are unusable and lose the whole stack on descent. Seals last one floor; two cleanses per shop, one condition step each.`)}</div><div class="grid grid-cols-2 md:grid-cols-3 gap-1">${rows || `<div class="text-[10px] text-slate-500 py-2 col-span-full">${currentLang === 'ja' ? '対象アイテムなし' : 'No eligible items'}</div>`}</div>
    </div>`;
