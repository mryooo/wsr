...
function generatedBoardScore(tubes, colors, capacity) {
    ...
    tubes.forEach(tube => {
        ...
        for (let i = 1; i < tube.length; i++) if (tube[i] !== tube[i - 1]) transitions++;
    });
    ...
    return (transitions * 5) + (dispersion * 3) + (mixedTubes * 4) + (Math.min(2, emptyTubes) * 24) - (completed * 18);
}
...
function buildGuaranteedBoard(colors, tubeCount, capacity, floor) {
    ...
    for (let attempt = 0; attempt < attempts; attempt++) {
        let tubes = solved.map(tube => [...tube]);
        const seen = new Set([solvedKey]);
        const moves = [];
        for (let step = 0; step < targetSteps; step++) {
            ...
            candidates.forEach(candidate => candidate.score = generatedBoardScore(candidate.next, colors, capacity) + Math.random() * 18);
        ...
        const certificateValid = validateGeneratedSolution(tubes, moves, solvedKey, capacity, counts);
        const hasOpeningMove = tubes.some((_, fromIdx) => tubes.some((__, toIdx) => generatedPour(tubes, fromIdx, toIdx, capacity, counts)));
        ...
        const candidate = {tubes, moves, score, quality, certificateValid, hasOpeningMove, completed, attempt: attempt + 1};
        if (certificateValid && hasOpeningMove && (!best || quality > best.quality)) best = candidate;
        const minimumScore = (colors.length * Math.min(4, capacity - 1) * 5) + (colors.length * 4);
        const minimumSolutionLength = Math.min(48, 14 + floor);
        if (certificateValid && hasOpeningMove && completed === 0 && score >= minimumScore && moves.length >= minimumSolutionLength) break;
    }
    ...
    return best || {tubes: solved, moves: [], score: 0, certificateValid: true, hasOpeningMove: false, completed: colors.length, attempt: attempts};
}
